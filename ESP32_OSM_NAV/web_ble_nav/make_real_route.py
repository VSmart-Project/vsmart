#!/usr/bin/env python3
"""Decimate the OSRM Bến Thành → Vũng Tàu geometry (osrm_bt_vt.json) to N points
by cumulative distance and emit a compact JS array (BT_VT_REAL) to embed in
index.html as the road-following fallback route."""
import json
import math

d = json.load(open("osrm_bt_vt.json"))
coords = d["routes"][0]["geometry"]["coordinates"]  # [[lon, lat], ...]
total = d["routes"][0]["distance"]
print("raw pts:", len(coords), "distance km:", round(total / 1000, 1))


def dist(a, b):
    R = 6371000.0
    la1, la2 = math.radians(a[1]), math.radians(b[1])
    dla = la2 - la1
    dlo = math.radians(b[0] - a[0])
    h = math.sin(dla / 2) ** 2 + math.cos(la1) * math.cos(la2) * math.sin(dlo / 2) ** 2
    return 2 * R * math.asin(math.sqrt(h))


cums = [0.0]
for i in range(1, len(coords)):
    cums.append(cums[-1] + dist(coords[i - 1], coords[i]))

N = 60
targets = [total * i / (N - 1) for i in range(N)]
out = []
j = 0
for t in targets:
    while j < len(cums) - 2 and cums[j + 1] < t:
        j += 1
    a, b = coords[j], coords[j + 1]
    d0, d1 = cums[j], cums[j + 1]
    f = (t - d0) / (d1 - d0) if d1 > d0 else 0
    out.append((a[1] + (b[1] - a[1]) * f, a[0] + (b[0] - a[0]) * f))  # (lat, lon)

print("decimated pts:", len(out))
js = "const BT_VT_REAL = [\n"
for i in range(0, len(out), 4):
    row = ", ".join("[%.6f, %.6f]" % p for p in out[i:i + 4])
    js += "  " + row + ",\n"
js += "];"
print(js)
