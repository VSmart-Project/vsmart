...
> **AWS CloudFormation** deployment for the complete Vsmart backend on AWS.
...
|---|---|---|
| **Cognito User Pool** | `Vsmart-UserPool` | Email-based login / registration / OTP |
| **DynamoDB** | `Vsmart-Devices` | Device metadata storage |
| **DynamoDB** | `Vsmart-DeviceState` | Current device state for phased realtime rollout |
| **AWS Location Service** | `Vsmart-Map/Tracker/GeofenceCollection` | Map tiles, real-time GPS tracking, geofences |
| **AWS IoT Core** | `Vsmart_UpdateLocationTracker` | MQTT broker & routing rule |
| **Lambda (Python)** | `Vsmart-IoTMessageProcessor` | Parses MQTT payloads → pushes to Tracker |
| **Lambda (Python)** | `Vsmart-GeofenceEventConsumer` | Validates ownership and publishes geofence events |
| **EventBridge + SQS** | `Vsmart-GeofenceEventRule` | Captures geofence breach events |
| **SNS** | `Vsmart-AntitheftAlerts` | Email alert on anti-theft trigger |
...
Run this from inside the `vsmart-infrastructure` directory:
...
| DynamoTableName | `Vsmart-Devices` |
| DeviceStateTableName | `Vsmart-DeviceState` |
| MapName | `Vsmart-Map` |
| RealtimeAppEventsTopicTemplate | `users/{userId}/events` |
| RealtimeDebugDeviceEventsTopicTemplate | `devices/{deviceId}/events` |
| RealtimeRecommendedClientIdPrefix | `Vsmart-mobile-` |
...
5. Under **Restrictions → Allow resources**, select the map: `Vsmart-Map`
...
aws location create-key `
  ...
  --restrictions AllowActions="geo:GetMapStyleDescriptor","geo:GetMapGlyphs","geo:GetMapSprites","geo:GetMapTile",AllowResources="arn:aws:geo:us-east-1:<ACCOUNT_ID>:map/Vsmart-Map" `
  --no-expiry `
  --region us-east-1 `
...
```powershell
copy ..\vsmart-web\.env.example ..\vsmart-web\.env
```
...
```powershell
aws cloudformation delete-stack --stack-name Vsmart-UnifiedStack
```
...
|---|---|---|
| `ProjectName` | `Vsmart` | Prefix for all resource names |
